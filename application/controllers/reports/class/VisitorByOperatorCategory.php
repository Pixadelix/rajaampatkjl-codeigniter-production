...
class VisitorByOperatorCategory {
	protected $sql;
	protected $year;
	protected $country;
	...
	public function __construct() {
		...
		$this->CI->data['PAGE_TITLE'] = 'Visitor by Operator Category';
		$this->CI->breadcrumbs->push('Visitor by Operator Category', '/report/visitor-by-opr-category');
		$this->CI->enqueue_resource('resource/script/adminlte/reports/js/visitor_by_operator_category.php');
...
SELECT oc.name,
  ...
  FROM `kjl_ticket_sales` s, `kjl_operators` o, `kjl_operator_categories` oc
 WHERE s.operator_id = o.id
   AND o.category_id = oc.id
 ...
 GROUP BY oc.id, year";
	...
	public function get_chart_data() {
		...
		foreach($rows as $row) {
			$row = (object) $row;
			$rgb = getColor($row->name.$row->operator_id, $row->operator_id.$row->name);
			$data[] = array(
				'label' => $row->name,
