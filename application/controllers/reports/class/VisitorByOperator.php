...
class VisitorByOperator {
	...
	public function __construct() {
		...
		$this->CI->data['PAGE_TITLE'] = 'Visitor by Operator';
		$this->CI->breadcrumbs->push('Visitor by Operator', '/report/visitor-by-operator');
		$this->CI->enqueue_resource('resource/script/adminlte/reports/js/visitor_by_operator.php');
...
SELECT o.name,
  ...
  FROM `kjl_ticket_sales` s, `kjl_operators` o
 WHERE s.operator_id = o.id
   AND year(s.ticket_start_date) = :year
   $country_filter
 GROUP BY s.operator_id, year";
	}
