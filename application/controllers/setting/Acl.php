...
class Acl extends RA_Controller {
...
*/		
		...
		$restricted_resource_role = Model\Setting\Acl_restricted_resource::result()->find_by_group_id($group_id)->to_array();
	...
	public function save()
	{
		...
		$this->db->query("delete from sys_acl_restricted_resource where group_id = $role_id");
		
		if($selected_resource) {
			$restrict_menu = Model\Setting\Acl_resource::result()->where_not_in('id', $selected_resource)->get()->to_array();
		} else {
			$restrict_menu = Model\Setting\Acl_resource::result()->all()->to_array();
		...
		if($restrict_menu){
			foreach($restrict_menu as $r) {
				if($r['id']) {
                    ...
                    $rr = Model\Setting\Acl_restricted_resource::make(array(
