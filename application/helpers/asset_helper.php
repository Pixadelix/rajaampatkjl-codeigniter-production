...
if ( ! function_exists('portfolio_url'))
{
    ...
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item('portfolio_path');
    }
}
...
/**
 * Get the Absolute Portfolio Path
 *
 * @access  public
 * @return  string
 */
if ( ! function_exists('portfolio_path'))
{
    function portfolio_path()
    {
        $CI =& get_instance();
        return FCPATH . $CI->config->item('portfolio_path');
    }
}
...
if ( ! function_exists('css'))
{
    ...
    {
		//echo strpos($file, "//").'='.$file."\n";
		$root_path = null;
		$version   = null;
		if ( false === strpos($file, 'http') && false === strpos($file, "//") ) {
			$root_path = css_url();
			if ( file_exists( css_path().$file ) ) {
				$version = '?v='.filemtime(css_path().$file);
			}
		}
		
        return '  '.'<link rel="stylesheet" type="text/css" href="' . $root_path . $file . $version . '" media="' . $media . '">'."\n";
    }
}
    ...
    function js($file, $atts = array())
    {
		...
		if ( false === strpos($file, 'http') && false === strpos($file, "//") ) {
			...
			if ( file_exists( js_path().$file ) ) {
				$version = '?v='.filemtime(js_path().$file);
			}
		}
        $element = '<script type="text/javascript" src="' . $root_path . $file . $version . '"';
 ...
 * Creates the <script> tag that links all requested resource file
 * @access  public
 * @param   string
 * @param 	array 	$atts Optional, additional key/value attributes to include in the SCRIPT tag
 * @return  string
...
{
    function style_resources($file, $atts = array())
    {
		$root_path = null;
		$version   = null;
		if ( false === strpos($file, 'http') && false === strpos($file, "//") ) {
			...
			if ( file_exists( BASEPATH.$file ) ) {
				$version = '?v='.filemtime(BASEPATH.$file);
			}
		}
        $element = '<link rel="stylesheet/less" type="text/css" href="' . $root_path . $file . $version . '"';
...
if ( ! function_exists('resources'))
{
    function resources($file, $atts = array())
    {
		$root_path = null;
		$version   = null;
		if ( false === strpos($file, 'http') && false === strpos($file, "//") ) {
