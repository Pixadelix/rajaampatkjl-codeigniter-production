...
(function($) {
	...
	function drawChart() {
		...
		var config = createConfig( { display: true }, 'Visitor by Gender');
	...
	function getData() {
		...
		var Report_Config = {
			...
			DATATABLE_CONFIG: {
				ajax: {
					url: '/report/get-data/visitor-by-gender',
					type: 'POST',
				...
				order: [ [14, 'desc']],
				columns: [
					{ title: "Gender", data: 'gender', sClass: 'nowrap' },
		...
		$.ajax({
			url: '/report/get-chart-data/visitor-by-gender',
			method: 'POST',
			data: { 'year': year, 'country': country },
			dataType: 'json',
