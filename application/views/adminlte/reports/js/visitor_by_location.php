...
(function($) {
	...
	function drawChart() {
		...
		var config = createConfig( { display: true }, 'Visitor by Location');
	...
	function getData() {
		...
		var Report_Config = {
			...
			DATATABLE_CONFIG: {
				ajax: {
					url: '/report/get-data/visitor-by-location',
					type: 'POST',
				...
				order: [ [14, 'desc']],
				columns: [
					{ title: "Location", data: 'name', sClass: 'nowrap' },
		...
		$.ajax({
			url: '/report/get-chart-data/visitor-by-location',
			method: 'POST',
			data: { 'year': year, 'country': country },
			dataType: 'json',
